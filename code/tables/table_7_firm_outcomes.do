...
$quietly {
    ...
    foreach depvar in `depvars' {
        ...
        esttab, varwidth(30) compress nogaps noconst noomitted nomtitles ///
                ...
                cells("b(fmt(%4.3f) star)" "t(par fmt(%3.2f))") ///
                ...
                indicate(/*"Firm Controls=change_size"*/ "Year = *year " "Lag Dep. Var.=l_d*", labels(Y N))

        if $writeout  ///
            esttab using "${tdir}/table_7_firm_outcomes_`depvar'.tex", replace booktabs ///
                compress nogaps nobase noomitted nomtitles noconst nonote label ///
                ${stars} eqlabels(none) collabels(none) ///
                stats(r2 N, fmt(%3.2f %8.0gc) labels("R$ ^2$" "Observations") ) ///
                cells("b(fmt(%4.3f) star)" "t(par fmt(%3.2f))") ///
                indicate("\midrule Lag Dep. Var.=l_d*", labels(Y N)) ///
                keep(${depvar} CMR*) ///
                rename(`rename') ///
                order(*${depvar}*) ///
                substitute("\*" " * " ///
